      ...
      if (parsedNum) {
        ...
        if (prevRow) {
          for (let a = j - numLen - 1; a < j + 1; a++) {
            const coordinates = `${i - 1},${a}`;
            if (prevRow[a] == "*") {
              const m = result2[coordinates];
              if (m) {
                m.push(num);
              } else {
                result2[coordinates] = [num];
              }
            }
          }
        }
        ...
        if (row[j - numLen - 1] == "*") {
          const m = result2[`${i},${j - numLen - 1}`];
          if (m) {
            m.push(num);
          } else {
            result2[`${i},${j - numLen - 1}`] = [num];
          }
        }
        num = "";
      }
...
let count = 0;
let arr = [];
Object.keys(result2).forEach((e) => {
  const arr = result2[e].map((a) => +a);
  if (arr.length == 2) {
    const product = arr[0] * arr[1];
    count += product;
  }
});
console.log(result2);
