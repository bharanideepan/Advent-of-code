...
{x=807,m=622,a=3419,s=2345}`.split("\n\n");

let flows = content[0].split("\n").reduce((acc, rule) => {
...
function getAcceptedcountFromRange(range) {
  let count = 1;
  for (const i of Object.values(range)) {
    count *= i[1] - i[0] + 1;
  }
  return count;
}

function run(range, start) {
  let count = 0;
  for (const i of flows[start]) {
    const { category, operator, rating, destination } = i;
    if (!category) {
      if (destination === "A") {
        count += getAcceptedcountFromRange(range);
      } else if (destination !== "R") {
        count += run(range, destination);
      }
    } else {
      const newRange = JSON.parse(JSON.stringify(range));
      let index = 0;
      if (operator === ">" && newRange[category][1] > rating) {
        newRange[category][0] = Math.max(newRange[category][0], rating + 1);
        if (destination === "A") {
          count += getAcceptedcountFromRange(newRange);
        } else if (destination !== "R") {
          count += run(newRange, destination);
        }
        range[category][1] = Math.min(range[category][1], rating);
      }
      if (operator === "<" && newRange[category][0] < rating) {
        newRange[category][1] = Math.min(newRange[category][1], rating - 1);
        if (destination === "A") {
          count += getAcceptedcountFromRange(newRange);
        } else if (destination !== "R") {
          count += run(newRange, destination);
        }
        range[category][0] = Math.max(range[category][0], rating);
      }
    }
  }
  return count;
...
console.log(
  run({ x: [1, 4000], m: [1, 4000], a: [1, 4000], s: [1, 4000] }, "in")
);
