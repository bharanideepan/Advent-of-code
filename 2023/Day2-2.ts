...
const y = `Game 4: 1 green, 3 red, 6 blue; 3 green, 6 red; 3 green, 15 blue, 14 red
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green`;
const z = `Game 1: 4 red, 5 blue, 4 green; 7 red, 8 blue, 2 green; 9 blue, 6 red; 1 green, 3 red, 7 blue; 3 green, 7 red
...
games.forEach((game) => {
  const [gameId, content] = game;
  const rounds = content.split("; ");
  let invalidGame = false;
  for (let roundIndex = 0; roundIndex < rounds.length; roundIndex++) {
    const round = rounds[roundIndex];
    const cubes = round.split(", ").map((a) => {
      const [count, color] = a.split(" ");
      return { [color]: +count };
    });
    for (let i = 0; i < cubes.length; i++) {
      const cube = cubes[i];
      const gameResult = result[gameId];
      if (gameResult) {
        if (gameResult.red < cube.red) {
          gameResult.red = cube.red;
        }
        if (gameResult.green < cube.green) {
          gameResult.green = cube.green;
        }
        if (gameResult.blue < cube.blue) {
          gameResult.blue = cube.blue;
        }
      } else {
        result[gameId] = {
          red: cube.red ?? 0,
  ...
  // if (!invalidGame) {
  //     validRounds.push(game)
  // const [gameName, id] = gameId.split(" ");
  // count += +id;
  // }
});
