...
...$.*....
.664.598..`;
const x = `...............................930...................................283...................453.34.............................867....282....
  ...
  for (let j = 0; j < row.length; j++) {
    ...
    if (!isNaN(item)) {
      ...
      if (parsedNum) {
        ...
        if (prevRow) {
          for (let a = j - numLen - 1; a < j + 1; a++) {
            numData[1].push(prevRow[a]);
          }
        }
        ...
        if (nextRow) {
          for (let a = j - numLen - 1; a < j + 1; a++) {
            numData[1].push(nextRow[a]);
          }
        }
        numData[1].push(row[j]);
        numData[1].push(row[j - numLen - 1]);
        result.push(numData);
        if (num == "757" || num == "661") {
          console.log(num, j - numLen - 1, j + 1);
          console.log(num, numData[1].join(""));
        }
        num = "";
      }
    }
...
let arr = [];
result.forEach((ele) => {
  const [num, elements] = ele;
  const flag = elements.some((item) => {
    const isNum = parseInt(item);
    return isNaN(isNum) && item !== ".";
  });
  if (flag) {
    count += +num;
    arr.push(+num);
...
console.log(count);
var fs = require("fs");
var data = fs.readFileSync("Day3output.txt", "utf8");
var output = data
  .toString()
  .split("\n")
  .map((e) => +e);
const present = output.every((e) => arr.includes(e));
console.log(present);
console.log(arr.length, output.length);
// Remove common items from array1
const resultArray1 = output.filter((item) => !arr.includes(item));

// Remove common items from array2
const resultArray2 = arr.filter((item) => !output.includes(item));

console.log(resultArray1); // [1, 2]
console.log(resultArray2); // [6, 7]
