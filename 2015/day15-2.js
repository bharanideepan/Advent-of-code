const calculateHighestCookieScoreWithCalories = (ingredients, targetCalories) => {
    let maxScore = 0;
    for (let i = 0; i <= 100; i++) {
        for (let j = 0; j <= 100 - i; j++) {
            for (let k = 0; k <= 100 - i - j; k++) {
                ...
                for (let m = 0; m < ingredients.length; m++) {
                    ...
                    calories += quantities[m] * ingredients[m].calories;
                }
                if (calories !== targetCalories) continue;
...
const highestScoreWithCalories = calculateHighestCookieScoreWithCalories(ingredients, targetCalories);
console.log(highestScoreWithCalories);
