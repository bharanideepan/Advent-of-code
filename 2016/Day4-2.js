function findNorthPoleSectorID(input) {
    // Helper function to decrypt a name
    function decryptName(encryptedName, sectorID) {
        return encryptedName
            .split('')
            .map(char => {
                if (char === '-') {
                    return ' '; // Convert dashes to spaces
                } else {
                    // Rotate the character by sectorID
                    const rotatedChar = String.fromCharCode(
                        ((char.charCodeAt(0) - 97 + sectorID) % 26) + 97
                    );
                    return rotatedChar;
                }
            })
    ...
    rooms.forEach(room => {
        ...
        const encryptedName = match[1];
        const sectorID = parseInt(match[2]);
        const checksum = match[3];

        // Remove dashes and calculate letter frequencies
        const letterCounts = {};
        encryptedName.replace(/-/g, '').split('').forEach(char => {
            letterCounts[char] = (letterCounts[char] || 0) + 1;
        });

        // Sort letters by frequency (desc) and alphabetically in case of ties
        ...
        // Compute the checksum and validate the room
        const computedChecksum = sortedLetters.slice(0, 5).join('');
        if (computedChecksum === checksum) {
            // Decrypt the name
            const decryptedName = decryptName(encryptedName, sectorID);
...
console.log(findNorthPoleSectorID(input)); // Replace input with actual data
