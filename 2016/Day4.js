function calculateSectorSum(input) {
    let sectorSum = 0;

    // Split the input into lines (each representing a room)
    const rooms = input.split("\n");

    rooms.forEach(room => {
        // Extract encrypted name, sector ID, and checksum
        ...
        // Remove dashes from the encrypted name and count letter frequencies
        const letterCounts = {};
        ...
        const sortedLetters = Object.keys(letterCounts)
            .sort((a, b) => {
                ...
                return letterCounts[b] - letterCounts[a]; // Frequency order
            });

        // Compute the expected checksum from the sorted letters
        const computedChecksum = sortedLetters.slice(0, 5).join('');

        // If the checksum matches, the room is real; add sector ID to the sum
...
console.log(calculateSectorSum(input)); // Output: 1514
