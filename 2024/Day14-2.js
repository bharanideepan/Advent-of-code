...
const printOutput = (input, seconds, timeout) => {
    let div = document.querySelector("div");
    setTimeout(() => {
        let container = document.createElement("div");
        let heading = document.createElement("h4");
        heading.textContent = seconds;
...
for (let i = 0; i < seconds; i++) {
    ...
    const grid = generateGrid([...map.keys()])
    const isValidGrid = isValid(grid)
    if (isValidGrid) {
        // printOutput(grid, i + 1, timeout)
        grid.map((row) => {
            console.log(row)
        })
        console.log(i + 1)
