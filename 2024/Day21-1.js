...
const getPaths = (grid, start, end) => {
    ...
    const findPaths = (start, end) => {
        ...
        return { arrows: pathsList.filter(e => e.length === min).map(e => e.join("")) };
    };
...
input.map((row) => {
    ...
    result2.map((r1) => {
        ...
        if (!minLength) {
            ...
            minLength = temp[0].length
            result3 = result3.filter(e => e.length <= minLength);
            result3 = [...result3, ...temp];
        }
    })
    output += parseInt(row.join("")) * result3[0].length
    console.log(row.join(""), result3[0].length)
})
console.log(output)
