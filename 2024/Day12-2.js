...
const calculateTopAndBottomSides = (grid) => {
    let topSides = [];
    let bottomSides = [];
    for (let i = 0; i < grid.length; i++) {
        const row = grid[i];
        const topRow = grid[i - 1];
        const bottomRow = grid[i + 1];
        let topSidesTemp = [];
        let bottomSidesTemp = []
        for (let j = 0; j < row.length; j++) {
            const topItem = topRow ? topRow[j] : 0;
            const item = row[j];
            const bottomItem = bottomRow ? bottomRow[j] : 0;
            if (item == 1) {
                if (topItem == 1) {
                    topSidesTemp.push([])
                } else {
...
const calculateSides = (coordinates) => {
    ...
    const { topSides, bottomSides } = calculateTopAndBottomSides(grid)
    const { topSides: leftSides, bottomSides: rightSides } = calculateTopAndBottomSides(transposeGrid(grid))
