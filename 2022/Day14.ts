...
for (let index = 0; index < input.length; index++) {
  ...
  rocks = [...rocks, ...pathList.map(x => x.join())]
}
...
for (let i = 0; i < width; i++) {
  ...
  for (let j = 0; j < height; j++) {
    const [x, y] = [i + xStart, j + yStart];
    if (rocks.includes(`${y},${x}`)) {
      count++
      cave[i][j] = '#';
    } else {
      cave[i][j] = '.';
...
const getValue = (x, y) => cave[x]?.[y];

const getEnd = (pos) => {
  const [x, y] = pos;
  const [dx, dy] = [x + 1, y]
  const downEl = getValue(dx, dy);
  if (downEl === undefined) {
    return [-1, -1]; //abyss
  }
  if (downEl === '.') { //check down
    ...
    const leftDownEl = getValue(lx2, ly2);
    if (leftDownEl === undefined) {
      ...
      const rightDownEl = getValue(rx2, ry2);
